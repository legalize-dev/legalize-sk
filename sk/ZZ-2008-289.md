...
koncovým zariadením elektronické zariadenie, ktoré umožňuje prístup do prostredia virtuálnej registračnej pokladnice a tlač pokladničných dokladov, dokladov podľa [§ 4a ods. 2 písm. b)](#paragraf-4a.odsek-2.pismeno-b), intervalovej uzávierky a prehľadovej uzávierky,
...
Na účely plnenia povinnosti podľa [§ 3 ods. 1](#paragraf-3.odsek-1) možno používať len virtuálnu registračnú pokladnicu podľa [§ 2 písm. b)](#paragraf-2.pismeno-b).
...
vytlačenie ochranného znaku a unikátneho identifikačného kódu pokladničného dokladu vygenerovaného virtuálnou registračnou pokladnicou len na pokladničnom doklade, a to až po vytlačení iných údajov uvedených v [§ 8 ods. 1](#paragraf-8.odsek-1), [2](#paragraf-8.odsek-2) a [7](#paragraf-8.odsek-7),
...
získať údaje potrebné na plnenie povinností vyplývajúcich z osobitných predpisov.[13)](#poznamky.poznamka-13)

(3)

Tlačiareň, ktorá bude komunikovať s virtuálnou registračnou pokladnicou, musí umožňovať vytlačenie tlačových výstupov čitateľných po dobu piatich rokov od konca kalendárneho roka, v ktorom boli vyhotovené. Obsah alebo usporiadanie údajov výsledných výstupov vytvorených virtuálnou registračnou pokladnicou nie je možné ďalej doplňovať alebo upravovať.

(4)

Používať iné klientske prostredie pre používanie virtuálnej registračnej pokladnice podnikateľom, okrem klientskeho prostredia podľa [§ 2 písm. b) druhého bodu](#paragraf-2.pismeno-b.bod-2), je zakázané.
...
Pokladničný doklad vyhotovený virtuálnou registračnou pokladnicou obsahuje údaje podľa odseku 1 okrem údaja podľa odseku 1 písm. e) a unikátny identifikačný kód vygenerovaný virtuálnou registračnou pokladnicou. Unikátny identifikačný kód je zobrazený v alfanumerickom tvare a zároveň ako čitateľný QR kód spracovateľný technickými zariadeniami.
...
Kópie paragónov a denné uzávierky podľa [§ 10 ods. 4](#paragraf-10.odsek-4), pokladničné doklady vyhotovené pri uplatnení postupu podľa [§ 10 ods. 5](#paragraf-10.odsek-5), doklady označené slovami „NEPLATNÝ DOKLAD“ a tlačové výstupy z dennej uzávierky je podnikateľ povinný uchovávať chronologicky usporiadané po dobu piatich rokov od konca kalendárneho roka, v ktorom boli vyhotovené.
...
Intervalová uzávierka obsahuje najmenej dátum a čas vyhotovenia, údaje uvedené v [§ 4 ods. 4 písm. a)](#paragraf-4.odsek-4.pismeno-a) a [b) druhom až ôsmom bode](#paragraf-4.odsek-4.pismeno-b.bod-2) uložené vo fiskálnej pamäti a súhrnné údaje alebo podrobné údaje uvedené v odseku 2 písm. h) až n) a p) až t). Intervalová uzávierka vyhotovená virtuálnou registračnou pokladnicou obsahuje kód virtuálnej registračnej pokladnice a údaje podľa odseku 2 písm. c) až f) a h) až t).
...
Ak podnikateľ chce opätovne používať virtuálnu registračnú pokladnicu, je povinný postupovať podľa [§ 7a](#paragraf-7a).

(4)

Pri úmrtí podnikateľa a pri zániku podnikateľa, ktorý bol zrušený bez likvidácie, je dedič alebo právny nástupca povinný ukončiť používanie virtuálnej registračnej pokladnice najneskôr v lehote podľa osobitného predpisu.[14)](#poznamky.poznamka-14)
...
nepoužil elektronickú registračnú pokladnicu alebo virtuálnu registračnú pokladnicu na evidenciu tržby podľa [§ 3 ods. 1](#paragraf-3.odsek-1) alebo prijme tržbu na základe dokladu vyhotoveného elektronickou registračnou pokladnicou alebo virtuálnou registračnou pokladnicou, ktorý nie je pokladničným dokladom,
...
na evidenciu prijatej tržby používa elektronickú registračnú pokladnicu, ktorá nespĺňa požiadavky podľa [§ 4 ods. 1 až 4](#paragraf-4.odsek-1) alebo virtuálnu registračnú pokladnicu, ktorá nespĺňa požiadavky podľa [§ 4a](#paragraf-4a),
...
zaeviduje tržbu v elektronickej registračnej pokladnici alebo vo virtuálnej registračnej pokladnici, ale neodovzdá pokladničný doklad podľa [§ 8 ods. 1](#paragraf-8.odsek-1), [2](#paragraf-8.odsek-2) a [7](#paragraf-8.odsek-7),

f)

odovzdá pokladničný doklad, doklad alebo paragón, ktoré nespĺňajú náležitosti podľa [§ 8 ods. 1](#paragraf-8.odsek-1), [2](#paragraf-8.odsek-2) a [7](#paragraf-8.odsek-7) alebo [§ 10 ods. 4](#paragraf-10.odsek-4),

g)

nemá knihu elektronickej registračnej pokladnice umiestnenú na predajnom mieste podľa [§ 2 písm. zd)](#paragraf-2.pismeno-zd),

h)

nezaeviduje v elektronickej registračnej pokladnici alebo vo virtuálnej registračnej pokladnici vklad hotovosti podľa [§ 3 ods. 3](#paragraf-3.odsek-3),
...
nepoužíva elektronickú registračnú pokladnicu alebo virtuálnu registračnú pokladnicu v súlade s [§ 3 ods. 6](#paragraf-3.odsek-6),
...
nezaeviduje v elektronickej registračnej pokladnici alebo vo virtuálnej registračnej pokladnici údaje z vyhotovených paragónov v lehote podľa [§ 10 ods. 5](#paragraf-10.odsek-5),
...
neoznámi ukončenie prevádzky elektronickej registračnej pokladnice podľa [§ 15 ods. 2](#paragraf-15.odsek-2) alebo ukončenie používania virtuálnej registračnej pokladnice podľa [§ 15a ods. 1](#paragraf-15a.odsek-1),
...
v rozpore s týmto zákonom umožní zmeniť alebo zmení údaje v elektronickej registračnej pokladnici, vo virtuálnej registračnej pokladnici, v kontrolnom zázname alebo umožní zmeniť alebo zmení obsah alebo usporiadanie údajov výsledných výstupov vytvorených virtuálnou registračnou pokladnicou,
...
neukončí prevádzku elektronickej registračnej pokladnice podľa [§ 15 ods. 5](#paragraf-15.odsek-5),
...
[§ 16a písm. g) až i)](#paragraf-16a.pismeno-g), [k)](#paragraf-16a.pismeno-k), [l)](#paragraf-16a.pismeno-l), [n)](#paragraf-16a.pismeno-n), [p) až r)](#paragraf-16a.pismeno-p) a [u) až x)](#paragraf-16a.pismeno-x) od 100 eur do 3 300 eur; pokutu za správny delikt podľa [§ 16a písm. h)](#paragraf-16a.pismeno-h) daňový úrad alebo colný úrad neuloží, ak daňovým úradom alebo colným úradom zistená celková suma vloženej hotovosti v elektronickej registračnej pokladnici alebo vo virtuálnej registračnej pokladnici okrem prijatej tržby podľa [§ 3 ods. 3](#paragraf-3.odsek-3) nepresiahne sumu 20 eur,

c)

[§ 16a písm. m)](#paragraf-16a.pismeno-m) od 20 eur do 100 eur pri druhom a ďalšom zistení porušenia; pri prvom zistení porušenia daňový úrad alebo colný úrad pokutu neuloží, ale podnikateľa vyzve na odstránenie nedostatkov pri označení tovaru alebo služby,
...
[§ 16a písm. ao)](#paragraf-16a.pismeno-ao) od 10 000 eur do 40 000 eur.
...
Spáchanie správnych deliktov podľa [§ 16a písm. al)](#paragraf-16a) a [an)](#paragraf-16a) sa považuje za osobitne závažné porušenie tohto zákona.
...
pri prvom opakovanom zistení porušenia podľa [§ 16a písm. al)](#paragraf-16a.pismeno-al) a [an)](#paragraf-16a.pismeno-an) uloží pokutu podľa odseku 2 písm. c) alebo písm. d) a môže podať návrh na zrušenie živnostenského oprávnenia k činnosti, pri ktorej došlo k porušeniu tohto zákona,[18a)](#poznamky.poznamka-18a)

b)

pri každom ďalšom zistení porušenia podľa [§ 16a písm. al)](#paragraf-16a.pismeno-al) a [an)](#paragraf-16a.pismeno-an) uloží pokutu podľa odseku 2 písm. c) alebo písm. d) a podá návrh na zrušenie živnostenského oprávnenia k činnosti, pri ktorej došlo k porušeniu tohto zákona.[18a)](#poznamky.poznamka-18a)

(6)

Daňový úrad a colný úrad sa o zistení porušenia podľa [§ 16a písm. y) až al)](#paragraf-16a.pismeno-y) a [an)](#paragraf-16a.pismeno-an) vzájomne informujú.
...
Colný úrad alebo daňový úrad, ktorý elektronickú registračnú pokladnicu zabezpečil, vydá rozhodnutie o prepadnutí elektronickej registračnej pokladnice do 15 dní odo dňa doručenia výsledkov technickej expertízy podľa odseku 2, ktorej výsledky preukázali, že došlo k naplneniu niektorého z dôvodov zabezpečenia podľa odseku 1.
