...
záväzné podmienky uvedené v právnych predpisoch, najmä v základných podmienkach dodávok a v štátnych a odborových normách,
...
Použitie cenových normatívov a parametrických spôsobov musí vopred schváliť príslušný cenový úrad. Vo všetkých prípadoch, keď použitie spôsobov uvedených v písmene b) a c) nie je pre tvorbu cien podľa odseku 3 schválené a nemožno použiť stavebnicový spôsob, sa uplatňuje kalkulačné porovnanie.

(5)

Kombinácie jednotlivých spôsobov tvorby cien v inej forme, než umožňujú ustanovenia [§ 24](#paragraf-24) a [26](#paragraf-26), môžno použiť len so súhlasom príslušného cenového úradu.
...
Spotreba priameho materiálu sa kalkuluje podľa platných cenových normatívov materiálovej spotreby, pokiaľ sa ich použitie pre tvorbu ceny nového výrobku ustanovuje, v ostatných prípadoch sa kalkuluje podľa platných celoštátnych alebo odborových technicko-hospodárskych noriem, a ak nie sú, kalkuluje sa na základe podnikových spotrebných materiálový noriem.

(2)

Priamy materiál sa oceňuje platnými veľkoobchodnými cenami (pri výrobkoch, ktoré majú určené zvýhodnené ceny podľa [§ 38](#paragraf-38), sa oceňuje základnými veľkoobchodnými cenami) alebo maloobchodnými cenami v prípadoch, keď je organizácia povinná nakupovať za tieto ceny; za podmienok uvedených v odseku 3 sa oceňuje plánovanými cenami podľa platných odvetvových, odborových alebo vnútropodnikových cenníkov.

(3)

Spôsob zostavenia a používania odvetvových alebo odborových cenníkov materiálov schvaľuje cenový úrad. Plánované ceny podľa vnútropodnikových cenníkov materiálov možno používať, pokiaľ boli zostavené v súlade s predpismi o účtovníctve a pokiaľ v účtovníctve zistené rozdiely medzi týmito cenami a skutočnými cenami (prípadne nákladmi podľa odseku[4)](#poznamky.poznamka-4) nepresahujú v uplynulom kalendárnom roku 1,5 %; v opačnom prípade treba upraviť vnútropodnikový cenník materiálov.

(4)

K platným veľkoobchodným cenám, prípadne maloobchodným cenám nemožno pripočítať a do plánovaných cien vnútropodnikových cenníkov materiálov zahŕňať podiel zásobovacej réžie, ani iné náklady obstarania (odbytové prirážky, náklady na prepravu, nakladanie, vykladanie a pod.), ktoré sa kalkulujú do nepriamych nákladov, s výnimkou schválených Federálnym ministerstvom financií po dohode s Federálnym cenovým úradom.

(5)

V cenovej kalkulácii sa nepripúšťa priemerovanie jednotlivých položiek spotreby priameho materiálu, pokiaľ nie je premietnuté priamo do vnútropodnikového cenníka. Príslušné cenové úrady môžu ustanoviť výnimky z tohto postupu.

(6)

Neplnohodnotný materiál, včítane vlastného odpadu, použitý ďalej ako plnohodnotný materiál, možno oceniť najvyššie cenou plnohodnotného materiálu zodpovedajúceho danému účelu použitia; prípadné náklady na úpravu, napr. na regeneráciu, sa nesmú v tomto prípade započítať do nákladov výrobkov.
...
Zisk v cene nového výrobku prepočítaný podľa [§ 20](#paragraf-20) a prípadne upravený podľa odseku 4 môže príslušný cenový úrad, prípadne národný výbor ďalej zvýšiť v prípadoch podľa odseku 6 písm. b) o 50 % úspory priameho materiálu a priamych miezd, najviac však o 10 % prepočtovej ceny nového výrobku, t. j. ceny bez započítania podielu z úspory do zisku.
...
Na zabezpečovanie potrebných cenových relácií sa okrem spôsobov tvorby cien uvedených v [§ 23 až 26](#paragraf-23) využíva
...
Cenové zvýhodnenie sa uplatňuje len pri tvorbe cien nových výrobkov a za podmienky, že rentabilita započítaná do ceny výrobkov uvedených v odseku 2 písm. a) až c) utvorenej niektorým zo spôsobov podľa [§ 23 až 26](#paragraf-23) neprekračuje už relatívne vyššiu rentabilitu zodpovedajúcu cenovému zvýhodneniu. Pri jednotlivom výrobku sa môže uplatniť cenové zvýhodnenie iba z jedného dôvodu, s výnimkou uvedenou v [§ 38 ods. 5](#paragraf-38.odsek-5). V prípadoch, keď nový výrobok vyhovuje podmienkam pre cenové zvýhodnenie z rôznych dôvodov, uplatňuje sa najvyššie zvýhodnenie.

(4)

Uplatňovanie cenového zvýhodnenia nesmie byť v rozpore s opatreniami na zabránenie rastu cien dodávok pre bytovú výstavbu,[7)](#poznamky.poznamka-7) na zabezpečovanie výroby a predaja spotrebných výrobkov nižších cenových polôh, ani s ďalšími opatreniami, ktoré prijali cenové úrady a ostatné príslušné orgány na podporu optimálnej štruktúry dodávok, požadovanej odberateľmi. Cenové zvýhodnenie sa môže viazať na dodržanie ďalších podmienok, najmä na zabezpečenie potrebného rozsahu dodávok jestvujúcich výrobkov bežnej akosti a technickej úrovne, štandardného vyhotovenia a nižšej cenovej polohy.
...
Dôsledky určenia zvýhodnených cien, ich prechodu na základné ceny a dôsledky cenového znevýhodnenia sa nebilancujú, ani nie sú predmetom centrálneho vyrovnania precenenia zásob, pokiaľ príslušné ústredné orgány neustanovia inak.
...
Cena technicky pokrokového výrobku sa môže zvýhodniť najviac

a)

o 40 % podielu prepočítaného ročného prínosu,[10)](#poznamky.poznamka-10) vznikajúceho pri použití nového výrobku u odberateľa, najviac však o 15 % základnej ceny,

b)

o 10 % základnej ceny v prípade, že pre objektívne dôvody nemožno urobiť prepočet prínosu vznikajúceho pri použití nového výrobku u odberateľa.
...
Po skončení doby technicko-ekonomickej životnosti výrobku alebo na základe rozhodnutia orgánov uvedených v [§ 122](#paragraf-122) a [126](#paragraf-126) o technickej zastaranosti výrobku je výrobná organizácia povinná navrhnúť zníženie ceny tak, aby rentabilita zastaraného výrobku bola nižšia než skutočná rentabilita organizácie v uplynulom roku, keď vyrábala zastaraný výrobok, najmenej však o 10 % ceny.
...
Cenové zvýhodnenie platí počas zaradenia výrobku do prvého stupňa akosti, najdlhšie však tri roky od dátumu účinnosti rozhodnutia o zaradení výrobku do prvého stupňa akosti.

(3)

Cena výrobku zaradeného do prvého stupňa akosti sa môže zvýhodniť

a)

podľa ustanovenia [§ 32 ods. 2 písm. a)](#paragraf-32.odsek-2), ak výrobok zabezpečuje ekonomický prínos aj odberateľovi,

b)

až o 10 % základnej veľkoobchodnej ceny v ostatných prípadoch.
...
ak cena bola určená pred vydaním rozhodnutia o zaradení do prvého stupňa akosti a nebola určená dočasná cena,

b)

ak ide o výrobky odvetvia potravín a pochutín,
...
5 % z veľkoobchodnej ceny, prípadne základnej veľkoobchodnej ceny, ak bol výrobok zaradený do tretieho stupňa akosti pri prvom alebo novom hodnotení, alebo ak bol výrobok zaradený do tretieho stupňa akosti na základe kontroly,

b)

20 % z veľkoobchodnej ceny, prípadne základnej veľkoobchodnej ceny, ak bol výrobok zaradený do tretieho stupňa akosti pri opakovanom hodnotení.
...
Módne novinky sa môžu dodávať za zvýhodnené ceny v čase a v množstvách ustanovených v príslušnom osvedčení, najdlhšie však jeden rok. V jednotlivých prípadoch sa môže na základe osvedčenia a so súhlasom nadriadeného orgánu dodávateľa a ministerstiev obchodu republík cenové zvýhodnenie výnimočne predĺžiť až na dva roky.
...
Ceny výrobkov zaradených do módnych noviniek alebo výrobkov luxusného vyhotovenia sa na čas platnosti osvedčenia dojednávajú dohodou medzi dodávateľom a odberateľom s prihliadnutím na podmienky realizácie, najmä ponuky a dopytu.
...
Na zabezpečenie žiadúceho vývoja úrovne cien v odboroch, kde sa uplatňujú módne novinky a výrobky luxusného vyhotovenia, ministerstvá priemyslu republík po dohode s ministerstvami obchodu republík a príslušnými cenovými úradmi určia v rámci svojej pôsobnosti na nasledujúci rok maximálny celkový podiel módnych noviniek a luxusných výrobkov na dodávkach do trhových fondov najneskoršie do 31. augusta bežného roka. Ministerstvá priemyslu republík po dohode s ministerstvami obchodu republík môžu v rámci tohto celkového podielu diferencovať podiely módnych noviniek a luxusných výrobkov pre jednotlivé odbory trojmiestnej nomenklatúry jednotnej klasifikácie odborov a výrobkov a pre výrobné hospodárske jednotky podľa podmienok výroby a potrieb vnútorného trhu.
...
Ustanovenia odsekov 1 a 2 sa nedotýkajú zliav podľa [§ 35](#paragraf-35) a zliav podľa osobitných predpisov.[14)](#poznamky.poznamka-14)
...
Predbežný cenový limit sa určuje v prípadoch, keď sa požaduje ako súčasť podkladov vymedzujúcich úlohu rozvoja vedy a techniky, ktorej výsledkom má byť nový výrobok.

(2)

Účelom predbežného cenového limitu je určiť pre etapu vývoja, prípadne výskumu nového výrobku predbežnú výšku budúcej ceny.

(3)

Predbežný cenový limit sa tvorí na základe

a)

ukazovateľov charakterizujúcich vzťah technických parametrov k platným cenám porovnateľných výrobkov v prípadoch, keď ide o obmenu výrobkov na doterajšej technickej úrovni,

b)

odvodenia zo zahraničných cenových relácií výrobkov rozhodujúcich zahraničných výrobcov, najmä v prípadoch, keď sa pri novom výrobku predpokladá uplatnenie celkom nových princípov riešenia, nových materiálov a pod.,

c)

rozboru efektívnosti použitia nového výrobku v porovnaní s porovnateľnými tuzemskými alebo dovážanými výrobkami.
...
Konečný cenový limit sa určuje po ukončení vývoja nového výrobku v prípade, že sa po schválení prototypu rozhodne o zavedení nového výrobku do výroby. Jeho účelom je pôsobiť na dosiahnutie potrebnej úrovne hospodárnosti výroby nového výrobku a prípadne spresniť predbežný cenový limit v súlade s dosiahnutými výsledkami vývoja.

(2)

Konečný cenový limit sa tvorí niektorým zo spôsobov tvorby cien.

(3)

Pri zložitejších a od širšej kooperácie závislých výrobkoch je subdodávateľ hlavných kompletizačných výrobkov alebo rozhodujúcich materiálov povinný na žiadosť dodávateľa finálneho výrobku zabezpečiť určenie ich konečných cenových limitov.

(4)

Návrhy konečných cenových limitov sa vypracúvajú pre nové výrobky,

a)

ktoré budú výsledkom riešenia štátnych a rezortných plánov rozvoja vedy a techniky,

b)

ktoré sa budú dodávať z nových výrobných kapacít získaných investičnou výstavbou, a to v rozsahu ustanovenom osobitným predpisom,

c)

ak tak ustanoví nadriadený orgán dodávateľa alebo ak ich požadujú hlavní odberatelia,

d)

ktoré určí príslušný národný výbor pre organizácie riadené alebo spravované (ďalej len „riadené“) národnými výbormi.

(5)

V prípadoch, keď odberateľ má záujem na znížení určeného konečného cenového limitu, môže sa s dodávateľom dohodnúť na uplatnení postupu pri tvorbe ceny v zmysle [§ 36](#paragraf-36).

(6)

Návrhy konečných cenových limitov podľa odsekov 4 a 5 sa nepredkladajú, ak cenu možno určiť priamo.

(7)

Určiť konečný cenový limit vyšší než predbežný cenový limit alebo určiť vyššiu veľkoobchodnú cenu, než je konečný cenový limit, môže len príslušný cenový úrad.
...
Výška priamych obchodných nákladov platných v zahraničí, o ktoré sa upravujú ceny obchodnej parity, sa preukazuje kalkuláciou zostavenou podľa príslušných predpisov a v súlade s platným kalkulačným vzorcom. Pri použití bežných cien sú organizácie zahraničného obchodu povinné preukázať tuzemskej odberateľskej alebo dodávateľskej organizácií odôvodnenosť kalkulovanej výšky priamych obchodných nákladov.
...
Dohodnuté kúpne ceny nemožno používať

a)

pri výrobkoch, ktorých ceny boli dojednané na obdobie platnosti dlhodobých dohôd v obchode so socialistickými krajinami podľa zásad tvorby cien platných medzi členskými krajinami Rady vzájomnej hospodárskej pomoci,

b)

pri vývozoch a dovozoch, pre ktoré platí v oblasti ekonomických nástrojov zahraničného obchodu podľa rozhodnutia Federálneho ministerstva financií režim mimoriadnych cenových prirážok a mimoriadnych cenových zrážok,

c)

pri dovozoch pre obchodné a odbytové organizácie, pokiaľ nie je ustanovené inak.
...
##### Ústredne riadené ceny dovozu

(1)

Na dovážané výrobky, najmä suroviny, materiály a potraviny, ktorých ceny podliehajú konjunkturálnym výkyvom a ktoré pritom významným spôsobom ovplyvňujú celkovú úroveň nákladov hlavných odberateľov, ustanovuje Federálny cenový úrad záväznú úroveň kúpnych cien, t.j. ústredne riadené ceny, a to v závislosti od dlhodobého vývoja zahraničných cien.

(2)

Ústredne riadenými cenami sú ceny za vymedzené skupiny výrobkov, na základe ktorých sa určujú jednotlivé ceny výrobkov podľa akostí a druhov tak, aby zodpovedali ustanovenej úrovni ústredne riadených cien podľa [§ 81](#paragraf-81).

(3)

Zaradenie výrobkov do ústredne riadených cien, spôsob tvorby, prerokovanie a výšku ústredne riadených cien ustanovuje Federálny cenový úrad. Spôsob vyrovnania rozdielov medzi určenými a skutočnými kúpnymi cenami ustanovuje Federálne ministerstvo financií.
...
Veľkoobchodné ceny výrobkov z dovozu sa určujú na základe kúpnych cien upravených o určené cenové prirážky alebo zrážky v zahraničnom obchode.
...
Kúpne ceny nemožno použiť pri dodávkach tuzemských dodávateľov organizáciám zahraničného obchodu určených pre tuzemskú investičnú výstavbu, ktorú uskutočňujú zahraniční dodávatelia. Tieto dodávky sa uskutočňujú za veľkoobchodné ceny.
...
Ceny neštandardných strojov a zariadení nezahrnuté do platných cenníkov alebo do zoznamu cien reprezentantov neštandardných výrobkov sa tvoria niektorým zo spôsobov podľa [§ 23 až 27](#paragraf-23).
...
veľkoobchodné ceny podľa [§ 9 ods. 1 písm. c)](#paragraf-9.odsek-1.pismeno-c) tvorené stavebnicovým spôsobom s použitím platných veľkoobchodných cien alebo schválených spôsobov ich tvorby pre opakovane dodávané štandardné zostavy prvkov a strojového zariadenia,
...
Pre ceny za rekonštrukcie, adaptácie, opravy a údržbu stavebných objektov a prevádzkových súborov platia ustanovenia [§ 54 až 57](#paragraf-54) a [§ 59](#paragraf-59) primerane.
...
niektorým zo spôsobov cenového porovnania k tuzemskému výrobku podľa [§ 23 až 26](#paragraf-23), prípadne podľa [§ 28](#paragraf-28)
...
porovnaním ceny a podstatných technických parametrov, úžitkových vlastností, kvality, vybavenia a dodacích podmienok dovážaného výrobku s porovnateľným tuzemským výrobkom a ďalších hľadísk uvedených v [§ 29](#paragraf-29); ak neexistuje tuzemský porovnateľný výrobok, možno porovnávať s obdobným dovážaným výrobkom,

b)

pri neporovnateľných výrobkoch na základe bežnej kúpnej ceny s pripočítaním, prípadne odpočítaním
...
Prostriedky z tvorby cenových rozdielov sa používajú na úhradu dôsledkov cenových opatrení v oblasti maloobchodných cien podľa osobitných predpisov[20)](#poznamky.poznamka-20) alebo rozhodnutí príslušných orgánov. Cenové rozdiely v maloobchodných cenách tuzemských i dovážaných výrobkov podľa odseku 1 sa zúčtovávajú s rizikovým fondom, v ktorom tvoria samostatnú zložku.
...
V prípade zmien v organizačnom začlenení výrobcov nie sú pre organizácie riadené odvetvovými ústrednými orgánmi záväzné ceny platné pre organizácie, ktorých ceny určujú národné výbory, a naopak, s výnimkou odseku 2 a [§ 78 ods. 3](#paragraf-78.odsek-3).

(4)

V prípadoch, v ktorých platí jednotnosť cien podľa odseku 2, určia príslušné národné výbory pri nových, opakovane poskytovaných službách a prácach prechodne platné ceny na 6 mesiacov a o ich určení súčasne informujú príslušný cenový úrad. Prechodne platné ceny sa neurčujú v prípadoch tvorby cien podľa [§ 24 až 26](#paragraf-24).
...
Náhradnými dielcami z hľadiska tvorby cien sú výrobky, ktoré sa podľa jednotnej klasifikácie priemyselných odborov a výrobkov označujú ako dielce (náhradné dielce) a iné výrobky určené príslušným cenovým úradom, pokiaľ sa tieto dielce a iné výrobky dodávajú pre náhradnú potrebu.

(2)

Ceny náhradných dielcov vyrábaných dodávateľom, ktorý súčasne vyrába výrobok, ku ktorému patria, pokiaľ už neboli určené podľa odseku 3, sa tvoria z čiastkových kalkulácií jednotlivých dielcov zostavených z operatívnej kalkulácie výrobku, ku ktorému patria; zisk sa započítava podľa percentnej sadzby určenej príslušným cenovým úradom, ak nie je určená, započítava sa vo dvojnásobnej percentnej výške zisku obsiahnutého v operatívnej kalkulácii príslušného výrobku, minimálne vo výške 25 %, najviac však 75 % zo spracovacích nákladov.

(3)

Ceny náhradných dielcov vyrábaných dodávateľom, ktorý nevyrába výrobok, ku ktorému patria, pokiaľ už neboli určené podľa odseku 2, sa tvoria individuálnou kalkuláciou.

(4)

Ak po ukončení výroby výrobku alebo pri rozšírení, prípadne prevedení výroby náhradných dielcov do iných podnikov zvýšený zisk započítaný do cien náhradných dielcov podľa odseku 2 nestačí na úhradu vyšších nákladov výroby, môže dodávateľ navrhnúť zmenu cien, doloženú náležitosťami podľa [§ 84](#paragraf-84).

(5)

Ceny náhradných dielcov k výrobkom, pre ktoré dodávateľ už nie je povinný zabezpečovať výrobu náhradných dielcov, a ceny náhradných dielcov dodávaných v skrátených lehotách pre havarijné opravy neštandardných strojov a zariadení sa dojednávajú dohodou dodávateľa s odberateľom. Rovnako sa tvoria ceny renovovaných náhradných dielcov s tým, že cena renovovaného náhradného dielca nesmie prevýšiť 80 % ceny nového náhradného dielca.

(6)

Príslušné cenové úrady ustanovia, v ktorých prípadoch

a)

treba tvoriť ceny nových náhradných dielcov cenovým porovnaním,

b)

môžu dodávatelia používať dvojaké ceny (nižšie ceny pre dielce na kompletáciu výrobku vyrábaného inou organizáciou a vyššie ceny pre dodávky náhradných dielcov).
...
Do kalkulácie podľa odseku 2 sa zahŕňa zisk maximálne vo výške

a)

25 % zo spracovacích nákladov pri hospodárskych organizáciách,

b)

15 % zo spracovacích nákladov pri príspevkových organizáciách.

Výška zisku podľa písmena a) a b) sa môže znížiť dohodou dodávateľa (riešiteľa) a zadávateľa.
...
miezd osádky podľa mzdových taríf platných pre odovzdávajúcu organizáciu, ak je obsluha základných prostriedkov osádkou[26)](#poznamky.poznamka-26) súčasťou zmluvy o dočasnom používaní.
...
Pri prevodoch výrobných programov medzi organizáciami, pre ktoré platí jednosť cien, sa nemenia ceny výrobkov, ktoré sú predmetom prevodu.

(2)

V prípadoch, keď vyhláška pripúšťa nejednotné ceny toho istého výrobku zúčastnených organizácií a nový výrobca požaduje vyššiu cenu, než za akú sa výrobok dosiaľ dodával, alebo zhoršuje kvalitatívne alebo dodacie podmienky pri danej cene, alebo navrhuje vo vzťahu k porovnateľnému výrobku vyššiu cenu nového výrobku, sú tieto organizácie povinné uviesť tieto skutočnosti v podkladoch pre rozhodnutie svojich nadriadených orgánov, príslušných na schválenie dohody o prevode výrobného programu.[29)](#poznamky.poznamka-29) Súčasťou týchto podkladov je aj vyjadrenie hlavných odberateľov k týmto okolnostiam prevodu výrobného programu.

(3)

Ak v prípadoch podľa odseku 2 hlavní odberatelia, prípadne koneční odberatelia nesúhlasia s vyššou cenou prevádzaného výrobku, sú orgány príslušné na schválenie dohody o prevode výrobného programu povinné vyžiadať si stanovisko príslušného cenového úradu, prípadne národného výboru k návrhu ceny ešte pred svojím súhlasom na uzavretie dohody o prevode výroby. K tomuto vyjadreniu predkladá organizácia, ktorá preberá výrobu alebo jej nadriadený orgán cenovému úradu návrh ceny aj vtedy, keď pôsobnosť v tvorbe cien v príslušnej skupine výrobkov má podľa vyhlášky alebo na základe poverenia iný orgán alebo organizácia. Uvedený postup sa vzťahuje aj na organizácie patriacej do cenovej pôsobnosti národných výborov.
...
Návrhy na ostatné zmeny cien, s výnimkou zmien maloobchodných cien[35)](#poznamky.poznamka-35) a zmien uskutočňovaných podľa [§ 30 až 40](#paragraf-30), musí obsahovať:
...
zdôvodnenie navrhovanej výšky ceny; v prípade, že sa popri základnej cene navrhuje cenové zvýhodnenie podľa [§ 30](#paragraf-30), treba zdôvodniť aj výšku zvýhodnenej ceny,
...
zdôvodnenie výberu iného porovnateľného výrobku, než ktorý sa zvolil pre návrh veľkoobchodnej ceny podľa [§ 63 ods. 4](#paragraf-63.odsek-4),

c)

zdôvodnenie navrhovanej maloobchodnej ceny (výšky dane z obratu, obchodnej ceny, obchodnej zrážky, prípadne cenového rozdielu); v prípadoch uvedených v [§ 89 ods. 1 písm. g)](#paragraf-89.odsek-1.pismeno-g) aj zdôvodnenie základnej maloobchodnej ceny,
...
Návrh maloobchodnej ceny spotrebného výrobku, predkladaný samostatne obsahuje aj náležitosti podľa [§ 89 ods. 1 písm. a), b), c), d)](#paragraf-89.odsek-1), [f), g), h), i)](#paragraf-89.odsek-1.pismeno-f) a [k)](#paragraf-89.odsek-1.pismeno-k).
...
pre ktoré neplatí jednotnosť cien podľa písmena a) sa obmedzujú na náležitosti podľa [§ 89 ods. 1 písm. a), b), c)](#paragraf-89.odsek-1), [e)](#paragraf-89.odsek-1.pismeno-e), [g), h)](#paragraf-89.odsek-1.pismeno-g) a [k)](#paragraf-89.odsek-1.pismeno-k) a podľa [§ 90 ods. 1 písm. a)](#paragraf-90.odsek-1), [c), d) a e)](#paragraf-90.odsek-1.pismeno-c).
...
Federálne ministerstvo palív a energetiky v spolupráci s Federálnym ministerstvom hutníctva a strojárstva, pokiaľ ide o veľkoobchodné ceny bansko-stavebných prác v podzemí,
...
nadriadenému orgánu hlavného odberateľa podľa [§ 94 ods. 2 písm. a) až d)](#paragraf-94.odsek-2), ak ide o ministerstvá stavebníctva, lesného a vodného hospodárstva, obchodu (v prípadoch spotrebného tovaru) a ministerstvá poľnohospodárstva a výživy republík,
...
orgánom a organizáciám povereným cenovou gesciou podľa [§ 130](#paragraf-130),
...
Predbežný cenový limit výrobku a jeho zmeny navrhuje organizácia, ktorá rieši úlohu plánu rozvoja vedy a techniky. Stáva sa záväzným vo výške uvedenej vo schválených podkladoch úlohy plánu rozvoja vedy a techniky, ktorých je súčasťou. Predbežné cenové limity a ich zmeny sa registrujú na príslušnom cenovom úrade tak, že do jedného mesiaca po schválení úlohy zašle riešiteľ príslušnému cenovému úradu závery úvodného oponentského konania, prípadne priebežného oponentského konania, pokiaľ má vplyv na zmenu predbežného cenového limitu.

(4)

Návrhy konečných cenových limitov výrobkov predkladajú príslušným cenovým orgánom ústredné orgány nadriadené dodávateľským organizáciám v prípadoch podľa [§ 48 ods. 4 písm. a), b) a c)](#paragraf-48.odsek-4) a dodávateľské (ak nie sú známe, riešiteľské) organizácie v prípadoch podľa [§ 48 ods. 4 písm. d)](#paragraf-48.odsek-4.pismeno-d).
...
Zo spôsobu zaokrúhľovania cien podľa odseku 1 môžu príslušné cenové úrady povoliť výnimky

a)

pri objemove významných výrobkoch hromadnej spotreby, ak by zaokrúhlenie spôsobovalo výraznejšie rozdiely v hospodárskych výsledkoch dodávateľov alebo odberateľov,

b)

keď to vyžaduje používaný spôsob tvorby cien,

c)

keď sa cena výrobkov určuje na rozdielne merné jednotky.
...
Cenovú dokumentáciu je orgán alebo organizácia, ktorá cenu navrhla, povinná uchovávať počas platnosti, podklady potrebné na určovanie cien do zmeny cien, najdlhšie však päť rokov od prvej realizácie výrobku.
...
Orgány a organizácie podľa [§ 108 ods. 2](#paragraf-108.odsek-2) (ďalej len „kontrolné orgány“) a kontrolované organizácie sa pri cenovej kontrole spravujú všeobecne platnými metodickými pokynmi pre kontrolnú činnosť[44)](#poznamky.poznamka-44) a ustanoveniami tejto vyhlášky.
...
nedodržania platnej ceny, ak nejde o zľavu podľa [§ 42](#paragraf-42),
...
účtovania väčšieho množstva výrobkov a prác, než je množstvo podľa [§ 59 písm. b)](#paragraf-59.pismeno-b), vo faktúre alebo v splátkovom liste,

g)

predaja za cenu neurčenú príslušným orgánom alebo príslušnou organizáciou,
...
Neoprávnene získané prostriedky podľa odseku 1 písm. d) sa vypočítavajú z rozdielu medzi cenou zodpovedajúcou podstatným kvalitatívnym a dodacím podmienkam dodaného výrobku a účtovnou cenou.
...
Úhrn neoprávnene získaných prostriedkov zistených iným kontrolným orgánom než orgánom uvedeným v odseku 1 určuje kontrolovaná organizácia buď vyčerpávajúcim spôsobom, alebo

a)

pri neštandardných strojárskych výrobkoch, v stavebnej výrobe, v organizáciách poskytujúcich práce a služby obyvateľstvu a v obchodných organizáciách výberovým spôsobom, ak tak rozhodne kontrolný orgán,

b)

v ostatných prípadoch spôsobom dohodnutým medzi kontrolným orgánom a kontrolovanou organizáciou, prípadne jej nadriadenou organizáciou alebo orgánom, a ak nedôjde k takej dohode, spôsobom určeným príslušným cenovým úradom po dohode s príslušným ústredným orgánom; v oblasti cenovej pôsobnosti národných výborov spôsob určí príslušný národný výbor.
...
Organizácia, ktorá získala neoprávnene prostriedky v prípadoch podľa [§ 112 ods. 1](#paragraf-112.odsek-1), je povinná ich odviesť ako dodatkový odvod vo výške a spôsobom ustanoveným osobitným predpisom.[47)](#poznamky.poznamka-47)
...
ceny, konečné cenové limity podľa [§ 48 ods. 4 písm. a) a b)](#paragraf-48.odsek-4) a [ods. 7](#paragraf-48.odsek-7), normatívy kalkulačných položiek a ich zmeny; zmeny maloobchodných cien určuje po dohode s cenovými úradmi republík,
...
určujú spôsoby tvorby cien, ceny, konečné cenové limity podľa [§ 48 ods. 4 písm. a) a b)](#paragraf-48.odsek-4) a [ods. 7](#paragraf-48.odsek-7), normatívy kalkulačných položiek a ich zmeny; zmeny maloobchodných cien sa určujú po dohode s Federálnym cenovým úradom,
...
určujú ceny riešenia úloh štátneho plánu rozvoja vedy a techniky; pri štátnych programoch technického rozvoja robia tak ministerstvá výstavby a techniky republík po dohode s Federálnym ministerstvom pre technický a investičný rozvoj,
...
konečných cenových limitov a ich zmien pre výrobky podľa [§ 48 ods. 4 písm. a) a b)](#paragraf-48.odsek-4) a [ods. 7](#paragraf-48.odsek-7),
...
Ministerstvá určujú ceny a zmeny cien za riešenia úloh rezortného rozvoja vedy a techniky, ktoré zadávajú, a sadzby nepriamych nákladov pre tvorbu cien výskumných a vývojových prác podľa [§ 71 ods. 2](#paragraf-71.odsek-2).

(4)

Ministerstvá rozhodujú o technickej zastaranosti výrobkov podľa [§ 31 ods. 3](#paragraf-31.odsek-3), ktoré dodávajú im podriadené organizácie, okrem prípadov, keď nedôjde k dohode s ústredným orgánom odberateľa o technickej zastaranosti výrobku.
...
Ministerstvá poľnohospodárstva a výživy republík určujú po dohode s príslušným cenovým úradom republiky veľkoobchodné ceny prác a služieb vykonávaných poľnohospodárskymi organizáciami výhradne pre potreby poľnohospodárskej výroby, s výnimkou generálnych opráv traktorov a mechanizačných prostriedkov a služieb vykonávaných strojovými a traktorovými stanicami a opravovňami poľnohospodárskych strojov, a s výnimkou služieb, ktorých ceny sa určujú podľa [§ 127 ods. 3 písm. a)](#paragraf-127.odsek-3), [ods. 4 písm. a)](#paragraf-127.odsek-4) a [ods. 5 písm. e)](#paragraf-127.odsek-5.pismeno-e).

(8)

Ministerstvá obchodu republík zabezpečujú po dohode s príslušnými odvetvovými ústrednými orgánmi žiadúcu skladbu sortimentu v jednotlivých cenových polohách pri spotrebných výrobkoch, najmä primeraný podiel výrobkov nízkych cenových polôh.
...
Vrcholné orgány tých družstevných a spoločenských organizácií, ktoré vyrábajú a dodávajú výrobky, majú pôsobnosti uvedené v [§ 126 ods. 1, 2, 3, 4](#paragraf-126.odsek-1), [10, 11, 12 a 13](#paragraf-126.odsek-10).

(2)

Zväzy spotrebných družstiev určujú maloobchodné ceny výrobkov, včítane výrobkov z dovozu, predávaných výhradne v sieti svojich organizácií, prípadne výrobkov, pri ktorých sú poverené určovať ceny, a predkladajú Federálnemu cenovému úradu návrhy obchodných zrážok pri týchto výrobkoch. Opatrenia na predaj podľa [§ 126 ods. 9](#paragraf-126.odsek-9) zabezpečujú pri výrobkoch predávaných súčasne v organizáciách štátneho obchodu po dohode s ústrednými orgánmi týchto organizácií.
...
určujú konečne cenové limity podľa [§ 48 ods. 4 písm. c)](#paragraf-48.odsek-4.pismeno-c),
...
určujú veľkoobchodné ceny dovážaných výrobkov, tvorené podľa [§ 53 ods. 1](#paragraf-53.odsek-1), s výnimkou ústredne riadených cien podľa [§ 52](#paragraf-52),
...
predkladajú nadriadeným ústredným orgánom návrhy konečných cenových limitov podľa [§ 48 ods. 4 písm. a) a b)](#paragraf-48.odsek-4) a [ods. 7](#paragraf-48.odsek-7),
...
maloobchodné ceny služieb poskytovaných im podriadenými podnikmi, pokiaľ sa neustanovuje inak,

c)

maloobchodné ceny porovnateľných priemyselných výrobkov z dovozu; maloobchodné ceny potravinárskych a tabakových výrobkov z dovozu len v rozsahu ustanovenom cenovými úradmi,
...
Výnimky z odseku 3 tvoria maloobchodné ceny, ktorých určenie si vyhradí príslušný cenový úrad, a maloobchodné ceny, ktoré určujú zväzy spotrebných družstiev podľa [§ 128](#paragraf-128), národné výbory, generálne riaditeľstvá výrobných organizácií podľa odseku 1 písm. ch) a organizácie podľa odseku 7.

(6)

Organizácie priamo riadené ústrednými orgánmi alebo národnými výbormi majú pôsobnosť podľa odseku 1 písm. b), d), e), f), g), i) až s), organizácie riadené národnými výbormi aj podľa [§ 48 ods. 4 písm. d)](#paragraf-48.odsek-4.pismeno-d) a [§ 129 ods. 7](#paragraf-129.odsek-7).

(7)

Odbytové a ostatné nevýrobné organizácie dodávajúce výrobky za maloobchodné ceny určujú maloobchodné ceny porovnateľných tuzemských a dovážaných priemyselných výrobkov predávaných výhradne v predajniach týchto organizácií iným socialistickým organizáciám pri bezhotovostnom predaji alebo občanom, s výnimkou

a)

maloobchodných cien určovaných spoločne s veľkoobchodnými cenami príslušným cenovým úradom,
...
Dodávatelia, ktorí boli povinní fakturovať podľa ustanovenia článku 3 ods. 4 úpravy Českého cenového úradu č. 1/1970 Ú. v. ČSR a úpravy Slovenského cenového úradu č. 15/1969 Ú. v. SSR nižšie ceny než ceny najvyššie prípustné, a odbytové organizácie, ktoré dosiaľ účtovali rozdielne ceny, musia ďalej fakturovať tým istým odberateľom tieto ceny, pokiaľ cenové úrady nerozhodli alebo nerozhodnú inak.

(3)

V prípadoch, keď sa pri výrobkoch s doterajšími maximálne limitovanými cenami dodávaných ďalej za maloobchodné ceny dojednala medzi dodávateľom a obchodnou organizáciou nižšia než určená cena, je táto nižšia cena porovnateľnou pre tvorbu cien nových výrobkov cenovým porovnaním. Výnimky povoľuje príslušný cenový úrad.
